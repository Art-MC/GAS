...
from utils import get_cell_info, load_and_prepare_xyz, get_nanocrystalline_grains, orient_and_shift_grain
...
def sample_grains(grains, min_dist, N, rng=np.random.default_rng(), max_attempts=32):
    ...
    centers = [np.mean(get_bounding_box(grain.alg_objects)[0], axis=0) for grain in grains]
...
def get_nanocrystal_seeds(domain, N_seeds, seed_crystal, min_dist, density, rng=np.random.default_rng(), **kwargs):
    # Get volumes for seed grains
    all_grains = get_nanocrystalline_grains(min_dist=min_dist, density=density, domain=domain)
    ...
    for grain in seed_grains:
        grain.add_generator(orient_and_shift_grain(base_crystal_generator.from_generator(), rng, crystal.lattice.a))

    return seed_grains

def add_seeds_to_amorphous_block(atoms, pbc, rng=np.random.default_rng(), tolerance=1e-5, **kwargs):
    ...
    scene = PeriodicScene(domain_cell=Voxel(atoms.get_cell()),
...
def main():
    ...
    config = {'seed_crystal':crystal_cif, # which crystal to use as a seed grain
